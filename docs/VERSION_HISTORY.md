...
## [feat/theme-code-color] - 2026-04-07
### Added
- **테마 마크다운 코드 색상 분리**: 테마의 기본 강조색(`accent`)과 별도로 인라인 코드 블록, 마크다운 코드 래퍼 라벨/테두리 색상을 별도 지정할 수 있는 `codeColor` 변수 도입.
- **테마 UI 항목 추가**: 색상 편집 모달 라인업에 `코드 텍스트색 (Code Color)` 항목 추가 및 실시간 프리뷰 동기화 기능.
- **기본 테마 업데이트**: `hpe_default`, `hpe_light`, `hpe_blue` `.slidetheme` 설정 파일에 `codeColor` 항목 기본값 업데이트 및 적용.

### Changed
- **다크모드 설정 이관**: 헤더의 개별적인 라이트/다크 모드 토글 버튼을 제거하고, `.slidetheme` 파일 내 `isLightMode` 속성으로 통합하여 테마에 종속되도록 구조 변경. 테마 모달에 "라이트 모드 사용" 옵션(체크박스) 추가.
