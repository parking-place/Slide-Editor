...
## [Unreleased] (feat/web-guide-ui)
### Added
- **테마 전환 플로팅 버튼**: 헤더에 고정된 인라인 버튼을 제거하고, 우측 하단 위로가기 버튼(bottom: 30px) 바로 위(bottom: 90px)에 원형 플로팅 버튼으로 변경. 스크롤 위치와 무관하게 항상 접근 가능.
- **동적 TOC 네비게이터**: 웹 가이드에 좌측 고정 사이드바(`guide-toc`) 추가. 대제목/중제목/소제목 계층트리 렌더링, 클릭 시 해당 섹션으로 스크롤, IntersectionObserver로 현재 위치 자동 하이라이팅.
- **다크모드 기본 적용**: `<body class="dark-mode">`로 변경하여 웹 가이드 최초 로드 시 다크 테마로 표시.
