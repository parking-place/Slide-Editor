...
## 4. 핵심 모듈 및 기능 (Functions)

### 4.1 데이터 로드 및 저장 (Data & File I/O)
...
### 4.2 화면 렌더링 및 UI 처리 (UI Rendering)
...
## 5. 향후 수정 시 고려사항 (Considerations)
1. **렌더링 방식 한계**: 현재 바닐라 JS의 `innerHTML` 조작으로 통째로 리렌더링(`renderPreview()`) 하는 방식이라 데이터가 매우 커질 경우 스크롤 위치 유지나 포커스 아웃 현상을 조심해야 합니다.
