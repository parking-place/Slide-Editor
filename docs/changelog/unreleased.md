...
- 가이드 HTML export 기본 파일명을 현재 프로젝트명 기준 `.html`로 생성하도록 조정했습니다.

## Fixed

- 이미지 업로드 직후 변환 완료 전까지 미리보기 영역이 비어 보이던 문제를 상태 표시 박스로 보완했습니다.
- 가이드 보기에서 네비게이터 현재 위치 하이라이트와 이미지 클릭 확대 기능이 빠졌던 회귀를 복구했습니다.
- 가이드 네비게이터를 에디터와 같은 크기와 상호작용 구조로 정리해 클릭 이동, 키보드 이동, 현재 위치 표시가 안정적으로 동작하도록 맞췄습니다.
- 프로젝트 매니저의 `Selected Project` 상세 텍스트가 테마를 따르지 않고 검정색으로 보이던 스타일 문제를 수정했습니다.
- 에디터 헤더의 프로젝트명 표시에서 괄호 안 내부 디렉터리명(`project id`)은 숨기고, 프로젝트명만 표시하도록 정리했습니다.
